...
class MapBuilder {
    ...
    private log = LogFactory.getLog(MapBuilder.class);
    ...
    void to(recip) {
        if(recip) {
            ...
            getMessage().to =[recip.toString()] as String[]
        }
    }
	...
	void attachBytes(String fileName, String contentType, byte[] bytes) {
		log.error "Attachments are not supported with Postmark.  Use of attachBytes will be ignored."
	}
    ...
    void to(Object[] args) {
        if(args) {
            ...
            getMessage().to = (args.collect { it?.toString() }) as String[]
        }
    }
    ...
    void headers(Map hdrs) {
		def headers = getMessage().headers
		if(!headers) {
			headers = hdrs
			return
		}
    ...
    void html(html) {
       	getMessage().html = html
    }

    void bcc(bcc) {
  		log.error "BCC is not supported with Postmark.  Use of bcc will be ignored."
    }
    ...
    void cc(cc) {
        ...
        getMessage().cc = [cc?.toString()] as String[]
    }
    void cc(Object[] args) {
		if (ConfigurationHolder.config.grails.mail.overrideAddress)
		   args = args.collect { ConfigurationHolder.config.grails.mail.overrideAddress }.toArray()
	
        getMessage().cc = (args.collect { it?.toString() }) as String[]
    }
    void cc(List args) {
		if (ConfigurationHolder.config.grails.mail.overrideAddress)
		   args = args.collect { ConfigurationHolder.config.grails.mail.overrideAddress }
